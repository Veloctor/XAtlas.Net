...
namespace ObjParser
{
	public class Obj
	{
		public readonly List<float3> Verts = new();
		...
		public override string ToString()
		{
			...
			foreach (float3 vert in Verts)
				sb.Append("v ").Appendfloat3(vert).Append('\n');
			if (HasUVs)
				foreach (float2 uv in UVs)
					sb.Append("vt ").Appendfloat2(uv).Append('\n');
			if (HasNormals)
				foreach (float3 norm in Normals)
					sb.Append("n ").Appendfloat3(norm).Append('\n');
	...
	{
		public static StringBuilder Appendfloat2(this StringBuilder sb, float2 v, char separator = ' ') =>
			sb.Append(v.x).Append(separator).Append(v.y);

		public static StringBuilder Appendfloat3(this StringBuilder sb, float3 v, char separator = ' ') =>
